...
const Categories = () => {
    ...
    const handleReorder = async (newOrder) => {
        try {
            await axios.post(`${window.react_app_url + window.product_category_url}/reorder`, { newOrder });
            return true;
        } catch (error) {
    ...
    const handleStatusChange = (id) => {
        ...
        axios.put(`${window.react_app_url + window.product_category_url}/status/${id}`, { status: newStatus })
            .then(res => {
                console.log(res);
    ...
    const deleteProductCategory = async (id) => {
        try {
            const response = await axios.delete(`${window.react_app_url + window.product_category_url}/${id}`);
            ...
            <div className="p-6 flex flex-col space-y-6 md:space-y-0 md:flex-row justify-between">
                ...
                <div className="flex flex-wrap items-start justify-end -mb-3 p-4">
                    <Link to="/admin/product-categories/add">
                        <Button label="Add new Product Categories" iconURL={<AiOutlinePlus />} />
                    </Link>
                </div>
            </div>
